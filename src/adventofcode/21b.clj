...
(def num-to-num (memoize (fn [input]
                           (num-to-num-helper input \A '()))))

(defn key-to-num [input]
  (key-to-num-helper input \A '()))

(defn split-at-A-helper [to-go current-list into-lists]
  (if (= 0 (count to-go)) into-lists
      (let [current-char (first to-go)
            is-A (= \A current-char)
            current-list (concat current-list (list current-char))
            into-lists (if is-A (conj into-lists current-list) into-lists)
            current-list (if is-A '() current-list)]
        (recur (rest to-go) current-list into-lists))))

(defn split-at-A [input]
  (split-at-A-helper input '() []))

(def calc-arrow-presses (memoize (fn [input times-remaining]
                                   (if (= 0 times-remaining) (count input)
                                       (let [split-input (split-at-A input)
                                             transformed (mapv num-to-num split-input)
                                             mapped-to-times (mapv #(calc-arrow-presses % (- times-remaining 1)) transformed)
                                             total (reduce + mapped-to-times)]
                                         total)))))

(defn calc-key-presses [input]
  (let [first-robot-press (key-to-num input)
        human-press (calc-arrow-presses first-robot-press no-of-times)]
    (println first-robot-press)
    human-press))

(defn -main [& args]
  (let [input-file (or (first args) "fixtures/input_test_21.txt")
        input (mapv #(into [] (seq (char-array %))) (str/split-lines (slurp input-file)))
        no_of_keypresses (mapv calc-key-presses input)
